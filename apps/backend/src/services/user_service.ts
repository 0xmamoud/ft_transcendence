...
import { UserProfile, UserPublicProfile } from "#types/user_type";
import Avalanche from "#services/blockchain_service";

export class UserService {
  constructor(private readonly app: FastifyInstance) {}

  async getMe(userId: number): Promise<UserProfile | null> {
    try {
  ...
  async getUserById(userId: number): Promise<UserPublicProfile | null> {
    try {
      const user = await this.app.db.user.findUnique({
        where: {
          id: userId,
